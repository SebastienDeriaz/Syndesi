...
Frame::Frame(Payload& payload, SyndesiID& id) {
    // Set the values
    //if (payload != nullptr && id != nullptr) {
        ...
        size_t packetLength =
            networkHeader_size + frameLength_size + frameLength;
        size_t pos = 0;
        ...
        pos += hton(reinterpret_cast<char*>(&frameLength),
                    _buffer->data() + pos, frameLength_size);
        ...
        // Write cmd_t
        pos += hton(reinterpret_cast<char*>(&command),
                    _buffer->data() + pos, command_size);
        // Write payload
